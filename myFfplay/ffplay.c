...
static int pic_contr_w;
static int pic_contr_h;
#endif
AVPacket flush_pkt;
...
static void video_image_display(VideoState *is)
{
    ...
    if (vp->bmp) {
		...
		//this means width depends on height
        ...
        rect.x = is->xleft + x;
        rect.y = is->ytop  + y;
        rect.w = width;    //the video screen width
        rect.h = height;   //the video screen height
...
static int decode_thread(void *arg)
{
    ...
    for(;;) {
        ...
        if (is->seek_req) {
            ...
            if(stream_index>=0){
                seek_target= av_rescale_q(seek_target,ap->time_base /*AV_TIME_BASE_Q*/, ic->streams[stream_index]->time_base);
            }
...
#ifdef VIDEO_PICTURE_SIZE_AUTO
int ffplay(char* argc, char **argv,const int picControl_w,const int picControl_h)
#else
...
#endif
{
	...
	//char **argv;
	//argv[0]="ffplay.exe";
	//argv[1]="rtsp://192.168.230.168:8557/PSIA/Streaming/channels/2?videoCodecType=H.264";
#ifdef VIDEO_PICTURE_SIZE_AUTO
	pic_contr_w=picControl_w;
	pic_contr_h=picControl_h;
#endif
    /* register all codecs, demux and protocols */
    av_register_all();

   // show_banner(program_name, program_birth_year);//show window detail
  ...
  //  parse_options(argc, argv, options, opt_input_file);
    ...
    if (!input_filename) 
	{
        //show_help();
       // exit(1);
	 input_filename="rtsp://192.168.1.168:8557/PSIA/Streaming/channels/2?videoCodecType=H.264";
   ...
   // flags = SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_TIMER;
    ...
    SDL_CreateThread((void*)event_loop, NULL);
	//event_loop();
